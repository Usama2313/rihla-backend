...
export default function Home() {
  ...
  const submitBooking = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedFlight) {
      ...
      setBookingRef(ref);
      const isUmrah = selectedBooking?.toLowerCase().includes("umrah");
      await saveRecord(isUmrah ? "umrah" : "travel", ref, `${bookingForm.fullName} ${bookingForm.surname}`.trim(), bookingForm.email, bookingForm.phone, {
        package: selectedBooking,
        passengers: bookingForm.passengers,
        nationality: bookingForm.nationality,
        passport: bookingForm.passport,
        passportExpiry: bookingForm.passportExpiry,
  ...
  const scanPassport = async (file?: File, target: "booking" | "umrah" = "booking") => {
    ...
    try {
      ...
      const cleanMrz1 = sanitizeMrzLine(mrz1).replace(/KBOCOR/g, "BOCOR");
    ...
    <main className={selectedFlight ? bookingStage === "fare" ? "fareDetailsView" : bookingStage === "trip" ? "tripSelectionView" : "partnerBookingView" : flightResults ? "flightResultsView" : "searchHomeView"}>
      ...
      <section className={`supplierSearch ${flightResults && activeService === "flights" ? "showingResults" : ""}`} id="book">
        ...
        {activeService === "flights" && <div className="servicePanel"><div className="panelTitle"><div className="journeyTypeTabs" role="group" aria-label="Journey type">{(["One way", "Round trip", "Multi-city"] as const).map((mode) => <button type="button" key={mode} className={flightMode === mode ? "selected" : ""} onClick={() => { setFlightMode(mode); if (mode !== "Round trip") setFlightSearch({ ...flightSearch, returnDate: "" }); }}>{mode}</button>)}</div></div><form className="flightForm enhancedFlightForm" onSubmit={searchFlights}>
          ...
          <div className="travellerFields"><label>Adults <small>12+ years</small><input min={1} max={9} type="number" value={flightSearch.adults} onChange={(e) => setFlightSearch({ ...flightSearch, adults: Number(e.target.value) })} /></label><label>Children <small>2–11 years</small><input min={0} max={12} type="number" value={flightSearch.children} onChange={(e) => setFlightSearch({ ...flightSearch, children: Number(e.target.value) })} /></label><label>Infants <small>Under 2 years</small><input min={0} max={9} type="number" value={flightSearch.infants} onChange={(e) => setFlightSearch({ ...flightSearch, infants: Number(e.target.value) })} /></label></div>

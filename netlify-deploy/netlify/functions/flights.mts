...
export default async function handler(request: Request, _context: Context) {
  ...
  try {
    ...
    const result = { supplier: sources.filter((source) => source.status === "connected").map((source) => source.name).join(" + ") || "No connected fare source", currency: xml.currency || body.currency || "USD", searchGuid: xml.searchGuid, resultCount: offers.length, offers, sources, testMode: duffel.offers.some((offer) => !offer.liveMode) };
    return new Response(JSON.stringify(result), { status: 200, headers: { "Content-Type": "application/json" } });
  } catch {
    return new Response(JSON.stringify({ error: "The flight suppliers are temporarily unavailable. Please try again." }), { status: 502, headers: { "Content-Type": "application/json" } });
  }
}

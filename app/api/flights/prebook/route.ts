...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.offerCode || !/^[0-9a-f-]{36}$/i.test(body.searchGuid || "") || !/^[A-Z]{3}$/.test(currency)) {
      return NextResponse.json({ error: "This fare is no longer linked to a valid supplier search. Please search again." }, { status: 400 });
    }
    ...
    if (fault) return NextResponse.json({ error: tag(fault, "Description") || tag(fault, "Text") || "The supplier could not revalidate this fare." }, { status: 502 });
    if (tag(xml, "Success").toLowerCase() !== "true") return NextResponse.json({ error: tag(xml, "ErrorString") || "This fare is no longer available. Please search again." }, { status: 409 });
    ...
    return NextResponse.json({ error: "The supplier is temporarily unavailable. Please try again." }, { status: 502 });
  }
}

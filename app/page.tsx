    ...
    <main className={selectedFlight ? bookingStage === "fare" ? "fareDetailsView" : bookingStage === "trip" ? "tripSelectionView" : "partnerBookingView" : flightResults ? "flightResultsView" : "searchHomeView"}>
      ...
      <section className={`supplierSearch ${flightResults && activeService === "flights" ? "showingResults" : ""}`} id="book">
        ...
        {activeService === "flights" && <div className="servicePanel"><div className="panelTitle"><div className="journeyTypeTabs" role="group" aria-label="Journey type">{(["One way", "Round trip", "Multi-city"] as const).map((mode) => <button type="button" key={mode} className={flightMode === mode ? "selected" : ""} onClick={() => { setFlightMode(mode); if (mode !== "Round trip") setFlightSearch({ ...flightSearch, returnDate: "" }); }}>{mode}</button>)}</div><small>Live supplier search powered by Rihla AI</small><span>XML Agency connected</span></div><form className="flightForm enhancedFlightForm" onSubmit={searchFlights}>
          ...
          <button type="submit" disabled={flightLoading} style={{ fontSize: '12px', padding: '2px 14px', width: 'auto', justifySelf: 'center', margin: '0 auto', display: 'block', borderRadius: '6px', minWidth: '130px' }}>{flightLoading ? "Searching every connected fare…" : `Search ${flightSearch.adults + flightSearch.children + flightSearch.infants} traveller${(flightSearch.adults + flightSearch.children + flightSearch.infants) > 1 ? "s" : ""} →`}</button>
